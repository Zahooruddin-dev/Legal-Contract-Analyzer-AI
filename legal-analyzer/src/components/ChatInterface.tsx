...
const MessageBubble = ({
	...
	const formatContent = (text: string) => {
		...
		formatted = formatted.replace(
			/`([^`]+)`/g,
			'<code class="bg-slate-700 px-1.5 py-0.5 rounded text-xs">$1</code>',
		);
		...
		formatted = formatted.replace(
			/(<li class="ml-4">.*<\/li>)/s,
			'<ol class="list-decimal ml-4 space-y-1">$1</ol>',
		);
	...
	return (
		<div className='p-4 bg-slate-900 border-t border-slate-800'>
			...
			<p className='text-[10px] text-center text-slate-600 mt-2'>
				AI generated responses may contain inaccuracies. Verify with legal
				counsel.
			</p>
		</div>
	);
	...
	<div className='h-full flex flex-col items-center justify-center text-center space-y-4 opacity-60'>
		...
		<div>
			...
			<p className='text-sm text-slate-500 max-w-xs mx-auto mt-1'>
				Ask questions about liability, termination clauses, or specific
				definitions within the document.
			</p>
		</div>
		<div className='flex flex-wrap justify-center gap-2 max-w-md'>
			<SuggestedQuery
				text='Summarize the indemnity clause'
