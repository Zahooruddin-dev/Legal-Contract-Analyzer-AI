	...
	if (typeof data === 'object' && !Array.isArray(data)) {
		...
		return (
			<div className='space-y-1'>
				...
				{d.role && (
					<span className='text-xs italic opacity-80 block'>({d.role})</span>
				)}
			...
			<div className='mt-6 pt-6 border-t border-slate-800'>
				...
				<div className='flex flex-wrap gap-2'>
					{keyTerms.slice(0, 6).map((term, i) => (
						<span
							key={i}
							className='px-2 py-1 bg-slate-800 border border-slate-700 rounded text-xs text-blue-300'
						>
							{typeof term === 'string'
								? term
								: (term.name ?? term.content ?? 'Term')}
						</span>
					))}
				</div>
			</div>
		...
		<div className='bg-slate-900 border border-slate-800 rounded-2xl p-10 shadow-2xl'>
			<div className='text-center mb-10'>
				...
				<p className='text-slate-400 max-w-md mx-auto'>
					Upload contracts, agreements, or legal briefs. Our AI identifies
					risks, obligations, and key terms instantly.
				</p>
			</div>
			...
			<div className='space-y-6'>
				<div className='relative group'>
					...
					/>
					<div
						className={`h-40 border-2 border-dashed rounded-xl transition-all flex flex-col items-center justify-center gap-3 ${file ? 'border-green-500/50 bg-green-500/5' : 'border-slate-700 group-hover:border-blue-500 group-hover:bg-slate-800/50 bg-slate-800/20'}`}
					>
						{file ? (
							<>
	...
	return (
		<div className='space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500'>
			<div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
				...
				<StatCard
					label='Key Terms'
					value={analysis.keyTerms?.length ?? 0}
					icon={FileCheck}
					color='blue'
				/>
				<StatCard
...
const LegalAnalyzerView: React.FC<LegalAnalyzerViewProps> = ({
	...
	const fallbackAnalysis: LegalAnalysis | null =
		analysis ??
		(text
			? {
					metadata: {
		...
		switch (activeTab) {
			...
			case 'results':
				return fallbackAnalysis ? (
					<ResultsView analysis={fallbackAnalysis} />
				) : null;
			case 'chat':
				return (
